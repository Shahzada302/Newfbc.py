...
def create_fake_profile(user_password, user_email):
    fake = Faker()
    password = user_password
    birthday = fake.date_of_birth(minimum_age=18, maximum_age=45)
    first_name = fake.first_name()
    last_name = fake.last_name()
    return user_email, password, first_name, last_name, birthday
...
""")
            ...
            print(f"{BOLD}{ORANGE}[!] Manual Verify Required: {YELLOW}{email}{RESET} — Login & Verify Email")
        else:
...
for i in range(count):
    ...
    for attempt in range(3):
        ...
        user_email = input(f"{BOLD}{BLUE}[📩] Enter Email For Account {i+1}: {RESET}")
        email, pw, fn, ln, bday = create_fake_profile(user_password, user_email)
        success = register_facebook_account(email, pw, fn, ln, bday)
        if success:
            break
        time.sleep(5)
    time.sleep(20)
